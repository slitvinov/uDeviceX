...
struct TransformedExtent
{
    ...
    void build_transform(const Extent extent, const int domain_extent[3])
    {
        ...
        for(int i = 0; i < 3; ++i)
        {
            float coeff = (i == 1) ? 2.0 : 0.5;
            transform[i][3] += coeff * maxlocalextent + drand48() * (domain_extent[i] - 2*coeff*maxlocalextent);
        }
    }
...
int main(int argc, const char ** argv)
{
    ...
    Extent extents[2] = {
            compute_extent("../cuda-rbc/sphere14.dat"),
            compute_extent("../cuda-ctc/sphere20.dat")
    };
    ...
    while (y < domainextent[1] - 4*scale)
    {
        if (drand48() > 0.1)
        {
            type = (int)(drand48() > (double)(results[1].size() + 0.5) / (results[0].size() + results[1].size() + 1.0));
            TransformedExtent tr(extents[type], domainextent);

            for (int i=0; i<4; i++)
                for (int j=0; j<4; j++)
                    tr.transform[i][j] = (i == j) ? 1 : 0;
            tr.transform[0][3] = 12*scale + 6*scale*(drand48() - 0.5);
            tr.transform[1][3] = y + 2*scale*(drand48() - 0.5);
            tr.transform[2][3] = 32*scale + 16*scale*(drand48() - 0.5);

            tr.xmin[0] = extents[type].xmin + tr.transform[0][3];
            tr.xmin[1] = extents[type].ymin + tr.transform[1][3];
            tr.xmin[2] = extents[type].zmin + tr.transform[2][3];
            tr.xmax[0] = extents[type].xmax + tr.transform[0][3];
            tr.xmax[1] = extents[type].ymax + tr.transform[1][3];
            tr.xmax[2] = extents[type].zmax + tr.transform[2][3];
            results[type].push_back(tr);
        }
        y += 24*scale;
        type = (type + 1) % 2;
    }
