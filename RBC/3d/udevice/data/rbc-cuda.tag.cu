...
/*s*/    void unitsSetup(float lmax, float p, float cq, float kb, float ka, float kv, float gammaC,
            float totArea0, float totVolume0, float lunit, float tunit, int ndens, bool prn)
    {
...
/*s*/    __global__ void areaAndVolumeKernel(float* totA_V)
    {
...
/*s*/    __device__ __forceinline__ float3 _fangle(const float3 v1, const float3 v2, const float3 v3,
...
/*s*/    __device__ __forceinline__ float3 _fvisc(const float3 v1, const float3 v2, const float3 u1, const float3 u2)
    {
...
/*e*/    }
    ...
    template <int nvertices>
    __device__
/*s*/    float3 _fangle_device(const float2 tmp0, const float2 tmp1, float* av)
    {
...
/*e*/    }
    ...
    template<int update>
/*s*/    __device__  __forceinline__  float3 _fdihedral(float3 v1, float3 v2, float3 v3, float3 v4)
    {
...
/*e*/    }
    ...
    template <int nvertices>
    __device__
/*s*/    float3 _fdihedral_device(const float2 tmp0, const float2 tmp1)
    {
        const int degreemax = 7;
        const int pid = (threadIdx.x + blockDim.x * blockIdx.x) / degreemax;
        const int lid = pid % nvertices;
